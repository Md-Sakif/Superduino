...
return {
  ...
  run = function(T)
    ...
    T.key("return")
    local progress_seen, bar_text = false, false
    T.key("return")
    ...
    T.wait_until(function()
      local p = v.panel and v.panel.progress
      if p then
        progress_seen = true
        if (p.done .. p.total):find("[", 1, true) then bar_text = true end
      end
      return v.panel == nil
    end, 20, "the install to finish")
    T.check(progress_seen, "download progress was reported")
    T.check(not bar_text, "arduino-cli's own text bar is not shown")
